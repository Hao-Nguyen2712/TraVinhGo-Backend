// Licensed to the .NET Foundation under one or more agreements.
// The .NET Foundation licenses this file to you under the MIT license.

using MongoDB.Bson.Serialization.Attributes;

namespace TraVinhMaps.Domain.Entities;
/// <summary>
/// Represent for destinationtype entity in the system.
/// </summary>
/// <seealso cref="TraVinhMaps.Domain.Entities.BaseEntity" />
public class DestinationType : BaseEntity
{
    /// <summary>
    /// Gets or sets the name.
    /// </summary>
    /// <value>
    /// The name.
    /// </value>
    [BsonElement("name")]
    public required string Name { get; set; }

    /// <summary>
    /// Gets or sets the update at.
    /// </summary>
    /// <value>
    /// The update at.
    /// </value>
    [BsonElement("updateAt")]
    [BsonDateTimeOptions(Kind = DateTimeKind.Utc)]
    public DateTime? UpdateAt { get; set; }

    /// <summary>
    /// Gets or sets a value indicating whether this <see cref="DestinationType"/> is status.
    /// </summary>
    /// <value>
    ///   <c>true</c> if status; otherwise, <c>false</c>.
    /// </value>
    [BsonElement("status")]
    public required bool Status { get; set; } = true;

    /// <summary>
    /// Gets or sets the marker identifier.
    /// </summary>
    /// <value>
    /// The marker identifier.
    /// </value>
    [BsonElement("markerId")]
    [BsonRepresentation(MongoDB.Bson.BsonType.ObjectId)]
    public required string MarkerId { get; set; }
}
